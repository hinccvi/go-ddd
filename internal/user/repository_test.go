...
func TestRepository(t *testing.T) {
	...
	// create
	createdUser, err := repo.Create(ctx, entity.CreateUserParams{
		Username: "user",
		Password: "secret",
	})
	assert.Nil(t, err)
	...
	// update
	updatedUser, err := repo.Update(ctx, entity.UpdateUserParams{
		ID:       user.ID,
		Username: "testuser",
		Password: "newsecret",
	})
	...
	// query
	albums, err := repo.Query(ctx, entity.ListUserParams{
		Limit:  10,
		Offset: 0,
	})
	assert.Nil(t, err)
	...
	testUser, err := repo.Create(ctx, entity.CreateUserParams{
		Username: "testuser1",
